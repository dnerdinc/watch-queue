...
async function fetchWatchProviders(filmTitle) {
  try {
    ...
    // Always build a JustWatch link — use TMDB's link if available, else construct one
    ...
    // flatrate = subscription, free = free with ads, ads = ad-supported
    const allProviders = [
    ...
    providers.forEach(p => {
      ...
      if (platformId) links[platformId] = jwLink;
    });
...
// ── FILM MODAL ────────────────────────────────
function FilmModal({ film, isWatched, note, rating, streamingLinks, friendsWatched, onToggle, onNoteChange, onRatingChange, onStreamingChange, onDelete, onClose, isCustom }) {
        ...
        <div style={{display:"flex",gap:18,alignItems:"center"}}>
          {feed.slice(0,6).map((item,i)=>(
            <div key={i} style={{fontFamily:"'Courier New',monospace",fontSize:"0.6rem",color:"#6e6e88",display:"flex",gap:5,alignItems:"center",whiteSpace:"nowrap"}}>
              ...
              <span style={{color:"#ede0cc",opacity:0.8}}>{item.display_name}</span>
...
export default function App() {
  ...
  async function loadUserData(){
    ...
    if(!data) return;
    const w={},n={},r={},sl={},custom=[],hidden={},orderMap={};
    data.forEach(row=>{
    ...
    setWatched(w);setNotes(n);setRatings(r);setStreamingLinks(sl);setCustomFilms(custom);setHiddenFilms(hidden);
  ...
  async function loadFriendsData(){
    const{data}=await supabase.from("user_films").select("film_title,user_id").eq("watched",true).neq("user_id",user.id);
    ...
    const map={};
    data.forEach(row=>{const name=nameMap[row.user_id]||"friend";if(!map[row.film_title])map[row.film_title]=[];if(!map[row.film_title].includes(name))map[row.film_title].push(name);});
    setFriendsData(map);
  }
  ...
  return(
    <div style={{background:BG,color:FG,fontFamily:"'Georgia','Times New Roman',serif",minHeight:"100vh",overflowX:"hidden",position:"relative",transition:"background 0.3s,color 0.3s"}}>
      ...
      <nav style={{
        ...
        background:`linear-gradient(to bottom, ${darkMode?"rgba(10,10,13,0.98)":"rgba(244,240,232,0.98)"} 0%, ${darkMode?"rgba(10,10,13,0.6)":"rgba(244,240,232,0.6)"} 70%, transparent 100%)`,
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-start",gap:2}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%"}}>
            {Array.from({length:22},(_,i)=>(
              <div key={i} style={{width:5,height:5,borderRadius:"50%",flexShrink:0,background:"#f5c518",boxShadow:"0 0 4px #f5c518,0 0 8px #f5c518",margin:"0 3px",animation:`bulb 1.4s ease-in-out ${(i%3)*0.47}s infinite`}} />
            ))}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{display:"inline-block",fontSize:"1.2rem",animation:"wobble 3s ease-in-out infinite",lineHeight:1}}>🍿</span>
            <span style={{fontFamily:"'Impact','Arial Black',sans-serif",fontSize:"clamp(1.1rem,2.5vw,1.6rem)",fontWeight:900,color:"#f5c518",textShadow:"1px 1px 0 #7a620a,2px 2px 0 rgba(120,96,10,0.4)",textTransform:"uppercase",letterSpacing:"-0.01em",lineHeight:1}}>The Watch Queue</span>
            <span style={{display:"inline-block",fontSize:"1.2rem",animation:"wobble 3s ease-in-out 1.5s infinite",lineHeight:1}}>🎬</span>
          </div>
        </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          {/* Progress pill */}
          <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(128,128,128,0.1)",border:"1px solid rgba(128,128,128,0.15)",borderRadius:20,padding:"4px 12px"}}>
            <div style={{width:60,height:4,background:"rgba(128,128,128,0.15)",borderRadius:2,overflow:"hidden"}}>
            ...
            <span style={{fontFamily:"'Courier New',monospace",fontSize:"0.6rem",color:"#f5c518",whiteSpace:"nowrap"}}>{watchedCount}/{totalFilms}</span>
          </div>

          {/* Dark/light mode */}
          <button onClick={()=>setDarkMode(d=>!d)} title={darkMode?"Light mode":"Dark mode"}
            style={{background:"rgba(128,128,128,0.1)",border:"1px solid rgba(128,128,128,0.2)",borderRadius:3,padding:"5px 10px",cursor:"pointer",fontSize:"0.85rem",transition:"all 0.15s",color:FG}}>
            {darkMode?"☀️":"🌙"}
          </button>

          <button onClick={copyLink} style={{fontFamily:"'Courier New',monospace",fontSize:"0.58rem",letterSpacing:"0.1em",textTransform:"uppercase",background:"transparent",border:"1px solid rgba(230,57,70,0.35)",color:"#e63946",borderRadius:3,padding:"5px 12px",cursor:"pointer",transition:"all 0.15s"}}
            onMouseEnter={e=>e.currentTarget.style.background="rgba(230,57,70,0.15)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
            Share
          </button>

          <button onClick={()=>setShowWatchParty(true)} style={{fontFamily:"'Courier New',monospace",fontSize:"0.58rem",letterSpacing:"0.1em",textTransform:"uppercase",background:"rgba(109,255,170,0.1)",border:"1px solid rgba(109,255,170,0.35)",color:"#6dffaa",borderRadius:3,padding:"5px 12px",cursor:"pointer",transition:"all 0.15s",display:"flex",alignItems:"center",gap:5}}
            onMouseEnter={e=>e.currentTarget.style.background="rgba(109,255,170,0.2)"} onMouseLeave={e=>e.currentTarget.style.background="rgba(109,255,170,0.1)"}>
            🎉 Watch Party
          </button>

          {/* User */}
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"4px 8px",borderRadius:4}}>
            <div style={{width:32,height:32,borderRadius:"50%",background:"linear-gradient(135deg,#793473,#f5c518)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Impact',sans-serif",fontSize:"0.9rem",color:"#0a0a0d",fontWeight:900,flexShrink:0}}>
